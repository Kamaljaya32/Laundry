...
export default function LaundryFormScreen() {
  ...
  const renderLaundryStep = () => {
    const totalHarga = items.reduce(
      (sum, item) =>
        sum + parseFloat(item.price || "0") * parseFloat(item.weight || "0"),
      0
    );
        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <TextInput
              ...
              style={styles.input}
              value={item.service}
              onChangeText={(text) => updateItem(index, "service", text)}
            />
            <View style={styles.row}>
              <TextInput
  ...
  const renderPaymentStep = () => {
    ...
    return (
      <View style={{ padding: 16, flex: 1 }}>
        ...
        <Text style={{ color: "#d00", marginTop: 12 }}>
          * Pilih metode pembayaran sesuai keinginan customer
        </Text>
        <TouchableOpacity
      ...
      <ScrollView style={{ padding: 16 }}>
        ...
        <View style={[styles.card, { marginTop: 16 }]}>
          ...
          <Text style={{ fontWeight: "bold", marginTop: 8 }}>
            Total Semua: Rp {totalHarga.toLocaleString("id-ID")}
          </Text>
        </View>
