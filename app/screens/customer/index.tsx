...
export default function CustomersScreen() {
  ...
  const del = (id: string, name: string) => {
    Alert.alert('Hapus?', `Hapus ${name}?`, [
      ...
      { text: 'Hapus', style: 'destructive', onPress: () => deleteDoc(doc(db, 'customers', id)) },
    ]);
  };
  ...
  const renderItem = ({ item }: { item: Customer }) => (
    <View style={s.card}>
      ...
      <View style={s.badgeRow}>
        {/* 1. Order */}
        <View style={s.badge}>
          <Ionicons name="cart-outline" size={14} color="#0066FF" style={{ marginRight: 4 }} />
        ...
        <TouchableOpacity
          style={s.badge}
          onPress={() =>
            router.push({ pathname: '/screens/customer/detail', params: { id: item.id } })
          }
        >
          <Ionicons
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            router.push({ pathname: '/screens/customer/edit/[id]', params: { id: item.id } })
          }
        >
          <Ionicons
            name="create-outline"
        ...
        {/* 4. Hapus */}
        <TouchableOpacity style={[s.badge, s.delBadge]} onPress={() => del(item.id, item.name)}>
          <Ionicons name="trash-outline" size={14} color="#FF3B30" style={{ marginRight: 4 }} />
  ...
  return (
    <View style={s.wrapper}>
      ...
      <TouchableOpacity style={s.fab} onPress={() => router.push('/screens/customer/add')}>
